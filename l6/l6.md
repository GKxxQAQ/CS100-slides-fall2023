...
- `p + i` returns the address `(char *)p + i * sizeof(T)`, which is `i * sizeof(T)` bytes away from `p`.
- If `p = &a[0]` (where `a` is an array of type `T [N]`), then
...
We can use pointers to traverse an array:s
...
- `p1 + 2 * N`, where `p` points to some element in an array of length `N`.
...
void copy_odd_reversed(int *from, int n, int *to) {
  for (int i = n - 1, j = 0; i >= 0; --i)
    if (from[i] % 2 == 1)
      to[i++] = from[j]; // What does this mean?
}
int main(void) {
  int a[5] = {1, 2, 3, 5, 6}, b[5];
