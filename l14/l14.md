...
There is a pointer called `this` in each member function of class `X` which has type `X *` or `const X *`, pointing to the *implicit object parameter*.
...
```cpp 
struct X {}; // No user-declared constructors.
...
- The `this` pointer: has type `X *` (`const X *` in `const` member functions). It points to the implicit object parameter of a member function.
