...
```c
int start_game_called = 0;
int end_game_called = 0;
int magic_power_used = 0;
int special_skill_used = 0;
// ...
```
...
```c
void start_game(Player *p1, Player *p2, int difficulty, GameWorld *world) {
  int called = 0; // Note that we can use a shorter name,
...
int main(void) {
  ...
  int d[100] = {};       // Since C23. All elements are initialized to zero.
}
```
...
1. The `swap` function on page 31 swaps the values of two integers. What if we want to swap the values of two pointers?
...
${}^{\textcolor{red}{2}}$ The "zero" value is not necessarily all-bit zero. For example, some special floating-point formats may represent `+0.0` not as all-bit zero. The null pointer value is not all-bit zero on some platforms, see details on [comp.lang.c FAQ](https://c-faq.com/null/machexamp.html).
